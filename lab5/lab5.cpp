...
int partitionLast(int array[], int left, int right)
{
   ...
   int pivot = array[right]; //fix this.
   //dont need to swap if choose last
...
int partitionMiddle(int array[], int left, int right)
{
   ...
   int pivot = array[(left + right) / 2]; //fix this.
...
int partitionMedianOf3(int array[], int left, int right)
{
   ...
   //as pivot
   int p[] = {array[left], array[right], array[(right + left) / 2]};
   insertionSort(p, 0, 2);
   int idx = right;
   int pivot = p[1]; //fix this.
   if (pivot != array[right])
   {
      pivot == array[left] ? idx = left : idx = (right + left) / 2;
