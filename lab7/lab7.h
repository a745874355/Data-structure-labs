...
class BST{
	...
	void remove(Node*& root, const T& data){
		if (root != nullptr){
			if (root->data_ > data)
			{
				remove(root->left_, data);
			}else if(root->data_ < data){
				remove(root->right_, data);
			}else{
				//remove
				if (root->left_ && root->right_) //Has two children
				{
					//switch the smallest node, in the tree of starting at root->right_, to current node
					//			6
					//	5						!15!
					//					14					18
					//									17
					...
					T temp = n->data_;
					remove(root_, n->data_);
					Node* newNode = new Node(temp, root->left_, root->right_);
					delete root;
					root = newNode;
				}else if(root->left_){ //Has only left child, simply connect it to the father node
					auto t = root->left_;
					delete root;
					root = t;
				}else if(root->right_){ //Has only right child, simply connect it to the father node
					auto t = root->right_;
					delete root;
					root = t;
				}else{ //nil node
	...
	void copy(Node*& destination, Node* res){
		if (res == nullptr)
		{
			destination = nullptr;
		}else{
			destination = new Node(res->data_, nullptr, nullptr);
			copy(destination->left_, res->left_);
			copy(destination->right_, res->right_);
		}
	}
